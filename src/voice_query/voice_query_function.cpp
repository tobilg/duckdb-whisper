...
static void RecordAndGenerateSQL(const WhisperConfig &config, int device_id, const std::string &ddl,
    ...
    HttpResponse response = client.Post(
        config.text_to_sql_url,
        json_body,
        config.text_to_sql_timeout
    );
...
void RegisterVoiceQueryFunctions(ExtensionLoader &loader) {
    ...
    TableFunction voice_query_with_device({LogicalType::VARCHAR, LogicalType::INTEGER},
                                           VoiceQueryExecute, VoiceQueryBind, VoiceQueryInit);
    ...
    // With model parameter
    TableFunction voice_query_sql_with_model({LogicalType::VARCHAR}, VoiceQueryExecute, VoiceQueryWithSqlBind, VoiceQueryInit);
    voice_query_with_sql_set.AddFunction(voice_query_sql_with_model);

    // With model and device_id parameters
    TableFunction voice_query_sql_with_device({LogicalType::VARCHAR, LogicalType::INTEGER},
                                               VoiceQueryExecute, VoiceQueryWithSqlBind, VoiceQueryInit);
