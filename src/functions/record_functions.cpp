...
static void WhisperMicLevelFunction(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        if (!pcm_data.empty()) {
            ...
            std::string result_str = "Peak: " + std::to_string(max_amplitude) +
...
void RegisterRecordFunctions(ExtensionLoader &loader) {
    ...
    record_set.AddFunction(ScalarFunction(
        {LogicalType::INTEGER},
        LogicalType::VARCHAR,
        WhisperRecordFunction
    ));
    ...
    record_translate_set.AddFunction(ScalarFunction(
        {LogicalType::INTEGER},
        LogicalType::VARCHAR,
    ...
    record_auto_set.AddFunction(ScalarFunction(
        {LogicalType::INTEGER, LogicalType::DOUBLE, LogicalType::VARCHAR, LogicalType::DOUBLE},
        LogicalType::VARCHAR,
        WhisperRecordAutoFunction
    ));
    ...
    mic_level_set.AddFunction(ScalarFunction(
        {LogicalType::INTEGER},
        LogicalType::VARCHAR,
        WhisperMicLevelFunction
    ));
