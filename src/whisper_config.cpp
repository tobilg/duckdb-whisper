...
WhisperConfig::WhisperConfig()
    : model(DEFAULT_MODEL), model_path(GetDefaultModelPath()), language(DEFAULT_LANGUAGE),
      threads(DEFAULT_THREADS), timestamps(DEFAULT_TIMESTAMPS),
      max_segment_length(DEFAULT_MAX_SEGMENT_LENGTH), translate(DEFAULT_TRANSLATE),
      device_id(DEFAULT_DEVICE_ID), max_duration(DEFAULT_MAX_DURATION),
      silence_duration(DEFAULT_SILENCE_DURATION), silence_threshold(DEFAULT_SILENCE_THRESHOLD),
      text_to_sql_url(DEFAULT_TEXT_TO_SQL_URL), text_to_sql_timeout(DEFAULT_TEXT_TO_SQL_TIMEOUT),
      voice_query_show_sql(DEFAULT_VOICE_QUERY_SHOW_SQL), voice_query_timeout(DEFAULT_VOICE_QUERY_TIMEOUT),
...
void WhisperConfigManager::RegisterSettings(DatabaseInstance &db) {
    ...
    // Model settings
    config.AddExtensionOption("whisper_model",
        "Whisper model name (e.g., tiny.en, base.en, small, medium, large-v3)",
        LogicalType::VARCHAR, Value(WhisperConfig::DEFAULT_MODEL));

    config.AddExtensionOption("whisper_model_path",
        "Path to store Whisper models",
        LogicalType::VARCHAR, Value(WhisperConfig::GetDefaultModelPath()));

    config.AddExtensionOption("whisper_language",
        "Target language code or 'auto' for detection",
        LogicalType::VARCHAR, Value(WhisperConfig::DEFAULT_LANGUAGE));

    config.AddExtensionOption("whisper_threads",
        "Number of processing threads (0 = auto-detect)",
        LogicalType::INTEGER, Value::INTEGER(WhisperConfig::DEFAULT_THREADS));

    // Recording settings
    config.AddExtensionOption("whisper_device_id",
        "Audio input device ID (-1 = system default)",
        LogicalType::INTEGER, Value::INTEGER(WhisperConfig::DEFAULT_DEVICE_ID));

    config.AddExtensionOption("whisper_max_duration",
        "Maximum recording duration in seconds",
        LogicalType::DOUBLE, Value::DOUBLE(WhisperConfig::DEFAULT_MAX_DURATION));

    config.AddExtensionOption("whisper_silence_duration",
        "Silence duration to stop recording (seconds)",
        LogicalType::DOUBLE, Value::DOUBLE(WhisperConfig::DEFAULT_SILENCE_DURATION));

    config.AddExtensionOption("whisper_silence_threshold",
        "Amplitude threshold for silence detection",
        LogicalType::DOUBLE, Value::DOUBLE(WhisperConfig::DEFAULT_SILENCE_THRESHOLD));

    config.AddExtensionOption("whisper_verbose",
        "Show status messages during recording and voice query operations",
        LogicalType::BOOLEAN, Value::BOOLEAN(WhisperConfig::DEFAULT_VERBOSE));

#ifdef WHISPER_ENABLE_VOICE_QUERY
    // Voice query settings
    config.AddExtensionOption("whisper_text_to_sql_url",
        "URL of the text-to-sql proxy service",
        LogicalType::VARCHAR, Value(WhisperConfig::DEFAULT_TEXT_TO_SQL_URL));

    config.AddExtensionOption("whisper_text_to_sql_timeout",
        "Timeout for text-to-sql proxy requests (seconds)",
        LogicalType::INTEGER, Value::INTEGER(WhisperConfig::DEFAULT_TEXT_TO_SQL_TIMEOUT));

    config.AddExtensionOption("whisper_voice_query_show_sql",
        "Show generated SQL in voice query output",
        LogicalType::BOOLEAN, Value::BOOLEAN(WhisperConfig::DEFAULT_VOICE_QUERY_SHOW_SQL));

    config.AddExtensionOption("whisper_voice_query_timeout",
        "Timeout for entire voice query operation (seconds)",
